...
public class DAO_ChuyenMon {
	...
	public List<ChuyenMon> getDanhSachChuyenMon() {
		...
		try {
			preparedStatement = connection.prepareStatement("SELECT * FROM CHUYENMON");
	...
	public boolean themChuyenMon(ChuyenMon chuyenMon) {
		...
		try {
			if (chuyenMon.getMoTa().isEmpty()) {
				preparedStatement = connection.prepareStatement("INSERT INTO CHUYENMON(MaCM, TenCM) VALUES (?, ?)");
				preparedStatement.setString(1, chuyenMon.getMaCM());
				preparedStatement.setString(2, chuyenMon.getTenCM());
			} else {
				preparedStatement = connection.prepareStatement("INSERT INTO CHUYENMON(MaCM, TenCM, [MoTa]) VALUES (?, ?, ?)");
				preparedStatement.setString(1, chuyenMon.getMaCM());
				preparedStatement.setString(2, chuyenMon.getTenCM());
	...
	public boolean suaChuyenMon(ChuyenMon chuyenMonMoi) {
		...
		try {
			if (chuyenMonMoi.getMoTa().isEmpty()) {
				preparedStatement = connection.prepareStatement("UPDATE CHUYENMON SET TenCM = ? WHERE MaCM = ?");
				preparedStatement.setString(1, chuyenMonMoi.getTenCM());
				preparedStatement.setString(2, chuyenMonMoi.getMaCM());
			} else {
				preparedStatement = connection.prepareStatement("UPDATE CHUYENMON SET TenCM = ?, MoTa = ? WHERE MaCM = ?");
				preparedStatement.setString(1, chuyenMonMoi.getTenCM());
	...
	public boolean xoaChuyenMon(ChuyenMon chuyenMon) {
		PreparedStatement preparedStatement = null;
		try {
			preparedStatement = connection.prepareStatement("DELETE CHUYENMON WHERE MaCM = ?");
			preparedStatement.setString(1, chuyenMon.getMaCM());
			int resultExecute = preparedStatement.executeUpdate();
			if (resultExecute > 0) {
				return true;
