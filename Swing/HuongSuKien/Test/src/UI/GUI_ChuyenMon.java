...
public class GUI_ChuyenMon extends JFrame implements ActionListener, MouseListener {
	private DAO_ChuyenMon daoChuyenMon;
	...
	public GUI_ChuyenMon() {
		...
		getContentPane().add(tabChuyenMon(), BorderLayout.CENTER);
	}

	public Component tabChuyenMon() {
		daoChuyenMon = new DAO_ChuyenMon();
		...
		txtMoTa.setHorizontalAlignment(SwingConstants.LEFT);
			...
			if (rowIndex > 0) {
				...
				if (JOptionPane.showConfirmDialog(this,
					...
					if (daoChuyenMon.xoaChuyenMon(chuyenMon)) {
						...
						JOptionPane.showMessageDialog(this, "Có nhân viên sở hữu chuyên môn này không thể xóa");
					}
				}
			} else {
