...
public class SinhvienDAO {
	...
	public List<Sinhvien> getSinhviens() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				Sinhvien sv = new Sinhvien(rs.getString("masv"), rs.getString("hoten"), rs.getString("gioitinh"),
						rs.getDate("ngaysinh"), rs.getString("DaiChi"), rs.getString("email"));
	...
	public List<Sinhvien> getSinhviens(String malop) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				Sinhvien sv = new Sinhvien(rs.getString("masv"), rs.getString("hoten"), rs.getString("gioitinh"),
						rs.getDate("ngaysinh"), rs.getString("diachi"), rs.getString("email"));
				Lophoc lophoc = new Lophoc(rs.getString("malop"));
				sv.setLophoc(lophoc);
				dssv.add(sv);
			}
