...
public class GuiThongTinSinhVien_JTree extends JFrame implements TreeSelectionListener, ActionListener{
	private JTree tree;
...
//	private LopHocDAO lopHocDAO;
	...
	public GuiThongTinSinhVien_JTree(String tieuDeCuaSo) {
	...
	public void addControl() {
		...
		/* Phan cay*/
		DefaultMutableTreeNode root = loadItemToTree();
		tree = new JTree(root);
		...
		pnCenter_North.setLayout(new BorderLayout());
		pnCenter_North.add(pnThaoTac, BorderLayout.SOUTH);
		pnCenter_North.add(b);


		sinhVienDAO = new SinhVienDAO();
		model = new SinhVienModel(sinhVienDAO.getDSSV());
		...
		pnBorderCha.add(tree, BorderLayout.WEST);
		pnBorderCha.add(pnCenter);
		add(pnBorderCha);
	...
	public DefaultMutableTreeNode loadItemToTree() {
		List<LopHoc> dsLop = new LopHocDAO().getDSLopHoc();
		DefaultMutableTreeNode root = new DefaultMutableTreeNode("Danh sách lớp học");
		for (LopHoc lopHoc : dsLop) {
			root.add(new DefaultMutableTreeNode(lopHoc.getTenLop()));
		}
		return root;
	}
	...
	public void valueChanged(TreeSelectionEvent e) {
		JTree nodeTree = (JTree) e.getSource();
		...
		for (LopHoc lop : dsLop) {
			if (lop.getTenLop().equals(selectedNodename)) {
				lopHoc = lop;
				break;
			}
		}
